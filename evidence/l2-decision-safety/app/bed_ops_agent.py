...
def decide_bed_disposition(
    *,
    ...
    if level == "NOW":
        extreme_saturation = (occ is not None and occ >= 99.0) or (beds is not None and beds <= 0 and (occ or 0) >= 95)
        if has_bed:
            disposition = "assign_bed"
            reason = f"Acute (NOW) patient and {beds} bed(s) free — assign immediately."
        elif extreme_saturation and (queue or 0) >= 7:
            disposition = "divert"
            reason = (
    ...
    elif los >= LONG_STAY_HOURS:
        if has_bed and (occ is None or occ < 80):
            disposition = "assign_bed"
            reason = (
                f"Long predicted stay {los:.0f}h but capacity available "
                f"({beds} beds, occupancy {occ}%) — admit rather than board."
            )
        elif beds is None or beds <= 2:
            ...
            disposition = "assign_bed"
            reason = f"Long stay {los:.0f}h with {beds} beds available — plan admit."
    elif has_bed:
        disposition = "assign_bed"
