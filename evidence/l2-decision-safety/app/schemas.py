...
class ERState(BaseModel):
    ...
    observed_at: str | None = Field(None, description="capacity snapshot timestamp (ISO8601)")
...
class SignalRouting(BaseModel):
    """Cheap signal layer before expensive retrieval/LLM — batch proof + per-query hint."""
    model_config = ConfigDict(extra="forbid")
    route_to_llm: bool = Field(..., description="True when this case stays on the full reasoning path")
    p_serious: float = Field(..., ge=0.0, le=1.0, description="cheap seriousness estimate from rules/signals")
    serious_floor_triggered: bool = Field(False, description="rule red-flag or ESI≤2 fired")
    nose_batch_reduction_pct: float | None = Field(None, description="NOSE batch eval: LLM call reduction @ recall floor")
    nose_batch_serious_recall: float | None = Field(None, description="NOSE batch eval: serious recall at operating point")
    nose_proof: str | None = Field(None, description="committed wiring proof JSON path in repo")
    method: str = Field(..., description="how routing decision was produced")
...
class AskResponse(BaseModel):
    ...
    signal_routing: SignalRouting | None = Field(
        None,
        description="NOSE-adjacent routing metadata — batch proof + per-query seriousness hint",
    )
    safety_decision: SafetyDecision = "ACT"
    safety_reason_code: str = "SAFETY_CHECKS_PASSED"
...
class CaseStatus(BaseModel):
    ...
    current_stage: str
    current_owner: str
    time_in_stage: int = Field(..., ge=0, description="seconds spent in the current stage")
