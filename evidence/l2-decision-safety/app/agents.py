...
def plan_agent_collaboration(
    *,
    ...
    has_bed = er_state is not None and (er_state.available_beds or 0) > 0
    los = prediction_signal.predicted_los_hours or 0
    bp = prediction_signal.bed_pressure_risk

    needs_bed_ops = (
        level == "NOW"
        or bp == "high"
        or los >= 36
        or (bp == "medium" and has_bed and los >= 8)
    )
    if needs_bed_ops:
